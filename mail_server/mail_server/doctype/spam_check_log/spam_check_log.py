...
class SpamCheckLog(Document):
	...
	def scan_message(self) -> None:
		...
		if not ms_settings.enable_spam_detection:
			frappe.throw(_("Spam Detection is disabled"))

		spamd_host = ms_settings.spamd_host
		spamd_port = ms_settings.spamd_port
		scanning_mode = ms_settings.scanning_mode
		hybrid_scanning_threshold = ms_settings.hybrid_scanning_threshold
	...
	def is_spam(self, message_type: Literal["Inbound", "Outbound"]) -> bool:
		...
		max_spam_score_field = (
			"outbound_spam_threshold" if message_type == "Outbound" else "inbound_spam_threshold"
		)
		max_spam_score = frappe.db.get_single_value("Mail Server Settings", max_spam_score_field, cache=True)
